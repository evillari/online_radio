...
class RequestController extends Controller
{
    ...
    public function getReco(Request $request)
    {
        $src = "http://api.dar.fm/topsongs.php?callback=json&q=" . $request->genre . "&partner_token=7824139131";
        $client = new Client();
        $result = $client->request('GET', $src);
        $reply = array(
            "statuscode" => $result->getStatusCode(),
    ...
    {
        $src =  "http://api.dar.fm/playlist.php?q=" . $param . "&callback=json&partner_token=7824139131";
        $client = new Client();
        $result = $client->request('GET', $src);
        return $result->getBody();
    }
